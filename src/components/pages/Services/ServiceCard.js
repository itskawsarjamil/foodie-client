...
const ServiceCard = ({ service }) => {
    // console.log(service);
    const { title, img, description, price, service_id, ingredient } = service;
    const printingredient = () => {
        // ingredient.map(ing => console.log(ing))
    }
    return (
        <div>
            <div className="flex w-full">
                <div className="relative flex flex-col items-start m-1 transition duration-300 ease-in-out delay-150 transform bg-white shadow-2xl rounded-xl md:w-80 md:-ml-16 md:hover:-translate-x-16 md:hover:-translate-y-8">
                    <PhotoProvider>
                        <PhotoView src={img}>
                            <img className="object-cover object-center w-full rounded-t-xl lg:h-48 md:h-36" src={img} alt="blog" />
                        </PhotoView>
                    </PhotoProvider>
                    ...
                    <div className="px-6 py-8">
                        ...
                        <Link to={`/services/${service_id}`}>
                            <button onClick={printingredient} className="mt-4 btn btn-outline btn-accent">Details</button>
                        </Link>
                    </div>
                </div>
            </div>
